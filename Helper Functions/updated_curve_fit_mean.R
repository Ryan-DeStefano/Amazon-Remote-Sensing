# Fits phenological curves for the mean evi value for each point in time within each polygon
# Need to create a new column in the dataset group.id which represents which polygon the group belongs to before running the function

updated_curve_fit_mean <- function(dt, si, window.yrs = 7, window.min.obs = 20, si.min = 0.15, 
  ...
  dt[, `:=`(obs.id, 1:nrow(dt))]
  obs.filtered <- c()
  n.sites <- length(unique(dt$group.id))
  ...
  if (test.run == TRUE) {
    if (n.sites < 9) {
      ...
      dt <- dt[group.id %in% sample(unique(dt$group.id), 9)]
    }
  }
  
  dt <- dt[, eval(c("group.id", 
                    "year", "doy", si)), with = FALSE]
  dt <- setnames(dt, si, "si")
  dt <- dt[order(group.id, doy)]
  dt <- dt[si >= si.min]
  dt[, `:=`(n.obs, .N), by = "group.id"]
  dt <- dt[n.obs > window.min.obs * 2]
  dt <- dt[, `:=`(n.obs, NULL)]
  
  rough.splines.dt <- dt[, .(spl.fit = list(stats::smooth.spline(doy, 
                                                                 si, spar = spar))), by = "group.id"]
  ...
  rough.spline.fits.dt <- rough.spline.fits.dt[, `:=`(doy, 
                                                      doy.rng), by = "group.id"]
  dt <- rough.spline.fits.dt[dt, on = c("group.id", "doy")]
  ...
  for (i in all.yrs) {
    ...
    focal.dt <- focal.dt[order(group.id, doy)]
    focal.dt <- focal.dt[, `:=`(n.obs.focal.win, .N), by = "group.id"]
    ...
    if (weight == TRUE) {
      ...
      splines.dt <- focal.dt[, .(spl.fit = list(stats::smooth.spline(doy, 
                                                                     si, w = weight, spar = spar))), by = "group.id"]
    } else {
      splines.dt <- focal.dt[, .(spl.fit = list(stats::smooth.spline(doy, 
                                                                     si, spar = spar))), by = "group.id"]
    }
    ...
    while (refitting == 1) {
      ...
      focal.dt <- focal.dt[, `:=`(c("spl.fit", "pcnt.dif"), 
                                  NULL)]
      refit.dt <- focal.dt[group.id %in% refit.sites]
      
      if (length(refit.sites) > 0) {
        spline.fits.dt <- spline.fits.dt[group.id %in% 
                                           refit.sites == FALSE]
  ...
  dt <- dt[order(group.id, year, doy)]
  data.table::setcolorder(dt, c("group.id", 
                                "year", "doy", "si", "spl.n.obs", "spl.fit", "spl.frac.max", 
                                "spl.fit.max", "spl.fit.max.doy", "si.adjustment", "si.max.pred"))
  n.sites <- length(unique(dt$group.id))
  
  if (n.sites > 9) {
    example.ids <- sample(unique(dt$group.id), 9, replace = FALSE)
  } else {
    example.ids <- sample(unique(dt$group.id), n.sites, 
                          replace = FALSE)
  }
  
  example.obs.dt <- dt[group.id %in% example.ids]
  example.curves.dt <- spline.dt[group.id %in% example.ids]
  
  fig <- ggplot(example.obs.dt, aes(doy, si)) + 
    labs(y = paste0("Landsat ", toupper(si)), x = "Day of Year") + 
    ggtitle("Nine random plantations") + 
    facet_wrap(~group.id, nrow = 3, ncol = 3, scales = "free_y") + 
...
updated_curve_fit_mean <- function(dt, si, group_id_to_plot, window.yrs = 7, window.min.obs = 20, si.min = 0.15, 
                                   spar = 0.78, pcnt.dif.thresh = c(-30, 30), weight = TRUE, 
                                   spl.fit.outfile = FALSE, progress = TRUE, test.run = FALSE) {
