...
# Initialize the vecotr of number of points to take from the brazil application
...
for (i in 1:nrow(peru)) {
  # Sample points from the i-th polygon
  pts <- st_sample(x = peru[i, ], size = n_pts_per_polygon[i])
...
```{r}
# Function to put sample points into groups based on the plantation theyw ere part of
...
# Find the mean evi value for each of group, day, year combo
...
```{r}
# Only keep the row with the minimum spline fit (predicted value) for each of the plantations (group.ids)
min_evi <- lsat.pheno.dt %>%
    group_by(group.id) %>%
    filter(spl.fit == min(spl.fit)) %>%
    select(group.id, min_evi = spl.fit, year)

# Sometimes there ar emultiple spline fit minimums, keept the row that has the year furthest back
min_evi <- min_evi %>% 
  group_by(group.id) %>% 
...
Playing with tracking the year that the value is most different from its typical value at that day of the year.

```{r}
average_evi <- lsat.pheno.dt %>%
  group_by(group.id, doy) %>%
  summarize(avg_evi = mean(spl.fit))

# Merge the average EVI values back to the original dataframe
a <- merge(lsat.pheno.dt, average_evi, by = c("group.id", "doy"))

# Calculate the difference between each year's EVI value and the average EVI value for that day of the year
b <- a %>%
  mutate(evi_difference = spl.fit - avg_evi)

# Find the year with the maximum absolute difference for each farm
year_of_max_difference <- b %>%
  group_by(group.id) %>%
  filter(abs(evi_difference) == max(abs(evi_difference))) %>%
  select(group.id, year)
