...
```{r}
combined_function <- function(joined_df, ALOS_df, group_ids) {
  ...
  # Filter for the years 2015 or later
  alos <- joined_df %>% 
    filter(year >= 2015)
  ...
  # Count the number of means there are per group.id, how many observations per plantation
  group_counts <- means %>%
    group_by(group.id) %>%
    summarize(count = n())
...
means <- combined_function(joined, ALOS.df, group.id_vector)
```
