...
n.pts_per_polygon <- 5  # Change this value as needed
...
n_pts_per_polygon <- c(rep(1, times=10))  # Example vector, replace with your desired values
...
```{r}
lsat.dt <- do.call("rbind", lapply(c("Top 10 50 points Chunk 1.csv", "Top 10 50 points Chunk 2.csv"), fread))
```
...
extract_group <- function(sample_id) {
  ...
  higher_order_group <- ((group_number - 1) %/% 50) + 1
...
# Fit phenological models (cubic splines) to each time series (Took out vi.min arg)
lsat.pheno.dt <- updated_curve_fit(lsat.dt, si = 'evi', spar = .6)
...
lsat.trnds.dt <- lsat_calc_trend(lsat.gs.dt, si = 'evi.max', 2010:2020, sig = 0.1)
