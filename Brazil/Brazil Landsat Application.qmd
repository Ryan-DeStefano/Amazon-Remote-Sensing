...
```{r}
middle_step <- st_cast(paracrop, "POLYGON")

# 2. Explode the multipolygon geometry
exploded_paracrop <- st_cast(middle_step, "POLYGON")
```
...
Checking locations to see how the multipolygons are working

```{r}
leaflet() %>%
  addProviderTiles('Esri.WorldImagery') %>%
  addPolygons(data = top_10_para$geometry[3:5],
              color = 'white',
              weight = 3) %>%
  addScaleBar(options = scaleBarOptions(imperial = FALSE))
```
...
for (i in 1:nrow(paracrop)){
  poly <- paracrop$geometry[i]
  if (st_geometry_type(poly) == "MULTIPOLYGON"){
    ...
    for (j in 1:length(polys)){
      ...
      pixel_list_test_poly <- lsat_get_pixel_centers(paracrop_poly, plot_map=FALSE)
